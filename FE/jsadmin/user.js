...
window.userModule = (function () {
  ...
  async function loadUsers() {
    ...
    if (!token) {
      ut.innerText = 'No token available. Please login as admin.';
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (window.adminApp && typeof window.adminApp.showToast === 'function') {
          adminApp.showToast('Saved successfully', 'success');
        }
      } else {
        document.getElementById('modalError').innerText = data.message || 'Error saving user'; document.getElementById('modalError').style.display = 'block';
        if (window.adminApp && typeof window.adminApp.showToast === 'function') {
          adminApp.showToast(data.message || 'Error saving user', 'error');
        }
      }
    } catch (err) {
      document.getElementById('modalError').innerText = 'Request failed: ' + err.message; document.getElementById('modalError').style.display = 'block';
    }
  }
  ...
  async function deleteUser(id) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        loadUsers(); if (window.adminApp && typeof window.adminApp.showToast === 'function') adminApp.showToast('Deleted successfully', 'success');
      } else {
        if (window.adminApp && typeof window.adminApp.showToast === 'function') {
          adminApp.showToast(data.message || 'Error deleting user', 'error'); alert('Error deleting user: ' + (data.message || 'Unknown error'));
        }
      }
    } catch (err) {
      if (window.adminApp && typeof window.adminApp.showToast === 'function') {
        adminApp.showToast(err.message, 'error'); alert('Request failed: ' + err.message);
      }
    }
  }
  ...
  async function init() {
    ...
    try {
      ...
      if (!data.success || !data.user || data.user.role !== 'admin') {
        ...
        localStorage.setItem('user', JSON.stringify(data.user)); await loadUsers(); if (window.adminApp && typeof window.adminApp.showSidebarUser === 'function') adminApp.showSidebarUser();
      }
    } catch (err) {
      console.error('Verify request failed', err); location = '/';
    }
  }
